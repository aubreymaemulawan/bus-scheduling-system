@extends('layouts.app')
@section('title','Dispatcher Report')

@section('content')
    <!-- Begin Page Content -->
    <div class="container-fluid">
        <!-- Page Heading -->
        <h1 class="h3 mb-2 text-gray-800">Dispatcher Report</h1>
        <!-- DataTables Example -->
...
@section('scripts')
    <script>
        ...
        $('#menu-reportDispatcher').addClass('active')
            ...
            function () {  
                $('#generate').on('click', function () {
                    ...
                    $('#gen-button').html('');
                        ...
                        $.ajax({
                            url: '{{ route('dispatcherGenerate') }}?active='+isActive+'&company='+companyId+'&from='+createdFrom+'&to='+createdTo,
                            type: 'get',
                            success: function (res) {
                                if(res === 0){
                                    ...
                                    $('#crd').append('<h1 style="text-align:center" class="h3 mb-2 text-gray-800"> <strong>ADMIN DISPATCHER REPORT</strong> </h1>');
                                    ...
                                    $('#crd').append('<p class="mb-2">Total Dispatcher : <strong style="color: #4e73df">'+res.length+'</strong></p>');
                                    if(isActive == 1){ $('#crd').append('<p class="mb-2">Dispatcher Status : <strong style="color: #4e73df">Active Dispatchers</strong></p>'); }
                                    else if(isActive == 2){ $('#crd').append('<p class="mb-2">Dispatcher Status : <strong style="color: #4e73df">Inactive Dispatchers</strong></p>'); }
                                    else{ $('#crd').append('<p class="mb-2">Dispatcher Status : <strong style="color: #4e73df">Active & Inactive Dispatchers</strong></p>'); }
                                    if(companyId == 0){
        ...
        function generatePDF(){
            ...
            html2pdf().from(output).save('dispatcher-report.pdf');
        }
    </script>
@endsection