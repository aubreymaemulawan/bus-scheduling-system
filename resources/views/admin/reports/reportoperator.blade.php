...
@section('content')
    <div class="container-fluid">
        <h1 class="h3 mb-2 text-gray-800">Operator Report</h1>
...
@section('scripts')
    <script>
        ...
        $('#menu-reportOperator').addClass('active')
            ...
            function () {  
                $('#generate').on('click', function () {
                    ...
                    $('#gen-button').html('');
                        ...
                        $.ajax({
                            url: '{{ route('operatorGenerate') }}?active='+isActive+'&company='+companyId+'&from='+createdFrom+'&to='+createdTo,
                            type: 'get',
                            success: function (res) {
                                if(res === 0){
                                    ...
                                    $('#crd').append('<h1 style="text-align:center" class="h3 mb-2 text-gray-800"> <strong>ADMIN OPERATOR REPORT</strong> </h1>');
                                    ...
                                    $('#crd').append('<p class="mb-2">Total Operator : <strong style="color: #4e73df">'+res.length+'</strong></p>');
                                    if(isActive == 1){ $('#crd').append('<p class="mb-2">Operator Status : <strong style="color: #4e73df">Active Operators</strong></p>'); }
                                    else if(isActive == 2){ $('#crd').append('<p class="mb-2">Operator Status : <strong style="color: #4e73df">Inactive Operators</strong></p>'); }
                                    else{ $('#crd').append('<p class="mb-2">Operator Status : <strong style="color: #4e73df">Active & Inactive Operators</strong></p>'); }
                                    if(companyId == 0){
                                    ...
                                    $('#tble').append('<th>Name</th>');
                                    $('#tble').append('<th>License No.</th>');
                                    $('#tble').append('<th>Contact No.</th>');
                                    $('#tble').append('<th>Age</th>');
                                    ...
                                    $.each(res, function (key, value) {
                                        ...
                                        $('#tble').append('<td>'+value.license_no+'</td>'); 
        ...
        function generatePDF(){
            ...
            html2pdf().from(output).save('operator-report.pdf');
        }
    </script>
@endsection