        ...
        <ul class="navbar-nav ml-auto">
            ...
            <li class="nav-item dropdown no-arrow">
                <a class="nav-link dropdown-toggle" href="#" id="userDropdown" role="button"
                    ...
                    @foreach($dispatcher as $disp)
                        @if($disp->name == Auth::user()->name)
                            @if($disp->profile_path=="")
                                <img class="img-profile rounded-circle" src="{{ asset('pub/img/default.jpg') }}">
                            @else
                                <?php
                                    $str = $disp->profile_path;
                                    $str = ltrim($str, 'public/');
                                ?>
                                <img class="img-profile rounded-circle" src="{{ asset('../storage/'.$str) }}">
                            @endif
                        @endif
                    @endforeach
                </a>
